...
import { Briefcase, Download, GraduationCap, Lightbulb, Cog } from 'lucide-react';
    ...
    <SectionWrapper id="resume" title="My Resume">
      <div className="text-center mb-12">
        <Button asChild size="lg" className="bg-accent text-accent-foreground hover:bg-accent/90">
          <a href="/resume.pdf" download="AlexJohnson-Resume.pdf">
      ...
      <div className="space-y-12">
        ...
        <div>
          <h3 className="text-2xl font-headline font-semibold mb-6 flex items-center text-primary-foreground">
            <Cog className="mr-3 h-6 w-6 text-accent" /> {/* Using Cog as a generic skills icon */}
            Skills
          </h3>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {resumeData.skills.map((skill) => (
                  <div key={skill.id} className="p-3 bg-muted/20 rounded-md text-center">
                    ...
                    <p className="text-xs text-accent">{skill.category}</p>
                  </div>
                ))}
              </div>
            </CardContent>
